...
DEFAULT_AZURE_OPENAI_MODEL = "azure-gpt-4o"
DEFAULT_REASONING_EFFORT = "medium"


class AzureOpenAIAugmentedLLM(AugmentedLLM[ChatCompletionMessageParam, ChatCompletionMessage]):
    """
    ...
    def __init__(self, provider_name: str = "AzureOpenAI", *args, **kwargs) -> None:
        ...
        super().__init__(*args, provider=Provider.AZUREOPENAI, **kwargs)
        ...
        self._reasoning = chosen_model and (
            chosen_model.startswith("azure-o3") or chosen_model.startswith("azure-o1") or chosen_model.startswith("azure-o4")
        )
        if self._reasoning:
            self.logger.info(
    ...
    def _initialize_default_params(self, kwargs: dict) -> RequestParams:
        """Initialize AzureOpenAI-specific default parameters"""
    ...
    def _api_key(self) -> str:
        ...
        if hasattr(config, "azureopenai") and config.azureopenai:
        ...
        if not api_key:
            raise ProviderKeyError(
                "AzureOpenAI API key not configured",
                "The AzureOpenAI API key is required but not set.\n"
    ...
    def _base_url(self) -> str:
        return self.context.config.azureopenai.base_url if self.context.config.azureopenai else None
    
    def _api_version(self) -> str:
        return self.context.config.azureopenai.api_version if self.context.config.azureopenai else "2024-12-01-preview"
    ...
    async def generate_internal(
        ...
        try:
            azureopenai_client = AzureOpenAI(api_key=self._api_key(), azure_endpoint=self._base_url(), api_version=self._api_version())
        ...
        # remove azure- prefix from model name
        ...
        for i in range(params.max_iterations):
            ...
            # if available_tools:
            #     arguments["parallel_tool_calls"] = False  # No supported by Azure
            ...
            executor_result = await self.executor.execute(
                azureopenai_client.chat.completions.create, **arguments
            )

            response = executor_result[0]

            self.logger.debug(
                "Azure OpenAI ChatCompletion response:",
                data=response,
            )
    ...
    async def structured(
        ...
        if not "AzureOpenAI" == self.provider:
        ...
        try:
            ...
            try:
                ...
                response = await self.executor.execute(
                    azureopenai_client.beta.chat.completions.parse,
                    model=model_name,
                    messages=messages,
